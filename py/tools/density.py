...
class DOSDirect(SelfEnergy):
    ...
    def F(self, x, y, ετ, uf):
        a = 2 * np.sqrt(x * y)
        b = -y / (4 * pow(x * y, 3 / 2))
        return (b * ((self.Fpart(x, y, a, uf) - self.Fpart(x, y, -a, uf)) - (
                    self.Fpart(x, y, a, 0) - self.Fpart(x, y, -a, 0))) + 1 / (a) * (
                            (self.FpartDer(x, y, True, uf) - self.FpartDer(x, y, False, uf)) - (
                                self.FpartDer(x, y, True, 0) - self.FpartDer(x, y, False, 0)))) / ετ
...
class DOSDirectNoInt(DOSDirect):
    ...
    def __call__(self, ε):
        ...
        ετ = (self.h) / (2 * τ)
        w = (ε * self.Ef) / (ετ)
        return -self.qmax * (self.funkciaPodIntegralom(self.qmax / 2, w0, ετ0)) + self.qmax * (
            self.funkciaPodIntegralom(self.qmax / 2, w, ετ))
